...
def simulation(param, pool=None):
        ...
        if pool:
                ...
                pool_outputs = pool.map(ModelRoger("normal",int(1000/num_sim)).make_mock,zip([np.array(param)]*num_sim,[cov]*num_sim))
                sim_output = np.concatenate(pool_outputs)
                return sim_output
        else:
                return ModelRoger("normal",1000).make_mock(param,cov)
...
def main():
        ...
        #make some fake data
        data = ModelRoger(model_type,nsamples).make_mock(zip(param, np.array([0.009,0.005,0.005,0.1])))
